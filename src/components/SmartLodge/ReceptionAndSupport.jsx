      ...
      <div className="main-container mx-auto py-10 lg:py-20 flex flex-col items-center gap-[60px]">
        <div className="flex flex-col items-center gap-4 text-center max-w-[959px]">
          ...
          <p className="font-ttCommonProNormal font-medium text-base leading-6 md:text-lg md:leading-[28px] text-white max-w-[650px]">
            SmartLodge automates the flow of room status and room reservation
            data between the front desk staff and other teams working to support
            camp operations.
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-10 items-center">
          <div className="flex flex-col bg-custom-heading max-h-max">
            {receptionAccordianData.map((item, index) => (
              <div
                className={`${
                  activeColor === item.id ? "bg-[#0F658D]" : ""
                } flex flex-col cursor-pointer border-b-[0.5px] border-[#00000080] border-opacity-10 transition-all duration-200`}
                key={item.id}
              >
                <div
                  onClick={() => {
                ...
                >
                  <div className="text-white font-ttCommonProNormal text-base w-[95%] pb-6 px-6">
                    {item.desc}
                  </div>
                </div>
              </div>
          ...
          <div className="relative w-full max-w-[500px] lg:max-w-[500px] xl:max-w-[662px] h-[300px] md:h-[350px] lg:h-[414px] mx-auto">
            {receptionImages.map((item) => (
              <div
                ...
                } absolute inset-0 w-full h-full transition-all duration-500 grid grid-cols-3 gap-4`}
                key={item.id}
              >
